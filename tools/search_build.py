...
# The marketing site's AI assistant (Website/api/chat.mjs) answers ONLY from text that is
# already published, and it retrieves that text from here -- the same extraction the search
# index uses, so an answer can never drift from what the page actually says. Regenerated by
# this script, which means an edit to any page updates the assistant automatically.
#
# It gets its own trimmed copy rather than reading assets/search/*.json, for two reasons:
#   - It must sit under lib/. .vercelignore keeps lib/ in the serverless bundle because
#     api/*.mjs import it at runtime; assets/ is served statically and is NOT bundled, so a
#     function cannot rely on reading it from disk.
#   - The assistant has no use for the long tail of one-line fragments that search still
#     wants, and every byte here is parsed on a cold start.
CHAT_OUT = WEBSITE / "lib" / "chat-index.json"
CHAT_BODY_CAP = 700   # the average section is ~540 chars, so this only bites on outliers
CHAT_MIN_BODY = 80    # shorter than this is a crumb or a stub -- it cannot answer anything
...
def chat_payload(core, blog):
    """Everything the assistant is allowed to know, in one bundled file.

    Carries llms.txt VERBATIM alongside the corpus. That file is the owner-authored
    summary -- the product description, the "what it deliberately does not do" list and
    the disclaimer -- and it is the factual core of the system prompt. It is embedded
    here rather than read from Website/llms.txt at runtime because llms.txt is a static
    asset: it is served to browsers, but it is NOT in the serverless bundle, so a
    function cannot count on opening it. Embedding also means an edit to llms.txt makes
    this file stale, and --check catches it -- so the assistant's own description of the
    product can never quietly drift from the one published for everyone else.
    """
    llms = (WEBSITE / "llms.txt").read_text(encoding="utf-8")
    return {"v": 1, "llms": llms, "records": chat_records(core, blog)}


def chat_records(core, blog):
    """The assistant's retrieval corpus: every searchable section, trimmed.

    Deliberately the same shape as the search records (u/a/t/h/b/k/c) so there is one
    mental model for both, and so a section can be cited back to the visitor as a real
    deep link rather than a claim with no source.
    """
    out = []
    for r in core + blog:
        body = r["b"]
        if len(body) < CHAT_MIN_BODY:
            continue
        if len(body) > CHAT_BODY_CAP:
            # Cut on a word boundary. A half-word reads as corruption to the model as
            # much as to a person, and costs a token to no purpose.
            body = body[:CHAT_BODY_CAP].rsplit(" ", 1)[0] + "…"
        out.append({"u": r["u"], "a": r["a"], "t": r["t"],
                    "h": r["h"], "b": body, "k": r["k"], "c": r["c"]})
    return out
...
def main():
    ...
    if checking:
        ...
        # The assistant's corpus goes stale the same way, and is worse when it does:
        # a stale search result looks visibly odd, but the assistant would state the old
        # text as fact, in its own voice, with a working link to a page that has changed.
        if not CHAT_OUT.exists():
            stale.append("lib/chat-index.json is MISSING - the site assistant would fall "
                         "back to llms.txt alone and answer far less")
        elif CHAT_OUT.read_text(encoding="utf-8") != serialise(chat):
            stale.append("lib/chat-index.json no longer matches the pages - the assistant "
                         "would state text that is no longer published")
        if stale:
    ...
    chat_text = serialise(chat)
    CHAT_OUT.write_text(chat_text, encoding="utf-8")
    summarise("lib/chat-index.json", chat["records"], chat_text)

    print("Done. %d searchable section(s); %d of them in the assistant's corpus."
          % (len(core) + len(blog), len(chat["records"])))
    return 0
