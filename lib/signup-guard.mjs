/* Signup guard — everything that decides whether an email address reaches the mailing list.
   ...
   Everything that survives goes to double opt-in, which is the real filter. Nothing here
...
/**
 * Run the full free screen. Never throws; never writes anything.
 *
...
export async function screenSignup(input) {
  const { honeypot, email, name, formToken, secret, checkToken = true } = input;
  ...
  if (isDisposableDomain(canon.domain)) {
  ...
  if (checkToken) {
    ...
    tokenState = v.state;
    if (v.state === 'forged') return { ok: false, reason: BLOCK_REASONS.TOKEN_FORGED, domain: canon.domain };
    if (v.state === 'too-fast') return { ok: false, reason: BLOCK_REASONS.TOKEN_TOO_FAST, domain: canon.domain };
  }
