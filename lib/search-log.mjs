...
export async function listBatches(days = 30) {
  const blobs = await listAll({ prefix: PREFIX, token: process.env.BLOB_READ_WRITE_TOKEN });
...
export async function aggregate(days = 30) {
  ...
  for (const b of batches) {
    let rec;
    try {
      // ⚠ Must be get() with the token, NOT fetch(b.downloadUrl). A private blob's URL returns
      // 403 to an unauthenticated request, so a plain fetch silently yields nothing and the
      // whole dashboard reads as "no searches yet" whatever is actually stored.
