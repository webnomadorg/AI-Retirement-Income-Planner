...
export async function claimConversation(hash, sid) {
  ...
  const prefix = `${CLAIM_PREFIX}${today()}/${hash}-`;
  try {
    const existing = await listAll({ prefix, token: blobToken() });
    /* ⚠ Undercounts a conversation started in the last few seconds, because a blob takes
       appreciable time to become listable. That is the right way to be wrong here: this is
       a backstop against someone clearing cookies, not the layer that bounds the bill, and
       refusing a real visitor is far more costly than allowing one extra. */
    if (existing.length >= IP_DAILY_CONVERSATIONS) return false;
  } catch (err) {
    console.error('[chat-quota] could not count conversations, allowing:', err?.message || err);
    return true;
  }
  try {
    await put(`${prefix}${sid}.json`, JSON.stringify({ at: new Date().toISOString() }), {
      access: 'private',
    ...
    // Already claimed by this same conversation: that is fine, it is not a new one.
