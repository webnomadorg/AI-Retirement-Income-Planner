...
/* ---- the owner's own answers ---------------------------------------------------------
   ...
   Questions the owner expects, with the wording they want used. ⚠ These are shown TO the
   model as preferred wording; they are never returned instead of it. Deciding "this
   question is that question" well enough to bypass the model is the same matching problem
   that defeats isContentGap() above — and a canned answer fired at the wrong question is
   worse than a generated one, because it is confident and off-target. Letting the model
   read them keeps rephrasing, follow-ups and "that is not quite what I meant" working.

   Selection is deliberately generous: a handful of entries costs a few hundred tokens, and
   the model ignores the ones that do not apply. Missing the relevant entry is the expensive
   failure, not carrying an extra. */
export function pickQa(question, qa, k = 3) {
  if (!Array.isArray(qa) || !qa.length) return [];
  const q = new Set(terms(question));
  if (!q.size) return [];
  const { df, docs } = index();
  const N = Math.max(docs.length, 1);
  /* ⚠ Prefix match, not equality. Exact tokens miss "macbook" against an entry written
     about a "mac", and "retiring" against "retire" — the single most common way a hand
     written entry fails to fire. Three characters is the floor, which does admit the odd
     loose pair like car/card; that is the right side of the trade here, because an extra
     entry costs a few tokens the model ignores while a missed one costs the whole point
     of the feature. Do NOT copy this looseness into retrieve(), where over-matching
     displaces a genuinely relevant section. */
  const qTerms = [...q];
  const hits = (t) => qTerms.some((x) => x === t
    || (x.length >= 3 && t.length >= 3 && (x.startsWith(t) || t.startsWith(x))));

  const scored = qa.map((e) => {
    let score = 0;
    for (const t of new Set(terms(e.q + ' ' + e.q + ' ' + e.a))) {
      if (!hits(t)) continue;
      // Rarer words are stronger evidence, exactly as in retrieve().
      const n = df.get(t) || 0;
      score += Math.min(Math.log(1 + (N - n + 0.5) / (n + 0.5)), IDF_CAP);
    }
    return { e, score };
  }).filter((x) => x.score > 0);
  scored.sort((a, b) => b.score - a.score);
...
- Never INVENT or improvise a refund, discount, extension or exception, never negotiate one, and never agree to one on the business's behalf. You MAY state a policy or an offer that appears in the site text or in the owner's written answers below, and point the visitor at the contact address to take it up. The distinction is between relaying something the owner has published and making something up: the first is your job, the second is never yours.
...
- Never rate, rank, criticise, recommend, or DESCRIBE another company's product - not its features, its pricing, its business model, its history or its name changes - even neutrally, even favourably, and even to say how it differs. You have no reliable information about anyone else's product: nothing about them is in the material you have been given, so anything you say about one comes from memory, may already be out of date, and is not yours to say. Reply that you do not comment on other products, describe what THIS one does, and let the visitor draw their own comparison.
...
- Never state a COUNT of plan-health checks from memory. This rule used to say "up to twelve" and the engine had grown to fourteen, so the assistant was quietly understating the product. The product facts page lists them by name; take the number from there, and say that how many apply depends on the plan because some are US-only and others activate only once the feature they test is configured.
...
[[page:products]] [[page:features]] [[page:faq]] [[page:how-it-works]] [[page:technical]] [[page:privacy]] [[page:blog]] [[page:contact]] [[page:product-facts]] — the named page. Use [[page:product-facts]] whenever you refer to the product facts page, which is where the plan-health checks and the model's documented simplifications are listed.
...
export function volatileBlock({ hits = [], sale = null, houseNotes = '', qa = [], now = new Date() } = {}) {
  const out = [];
  out.push('=== TODAY ===');
  ...
  if (Array.isArray(qa) && qa.length) {
    out.push('', '=== ANSWERS THE OWNER HAS ALREADY WRITTEN ===');
    out.push('If one of these fits what was asked, answer with it — keep the wording close to'
      + ' theirs rather than paraphrasing, and adapt only as much as the question requires.'
      + ' They still do not override any rule above. If none fits, ignore them entirely.');
    qa.forEach((e, i) => {
      out.push('', `(${i + 1}) Q: ${e.q}`);
      out.push(`    A: ${e.a}`);
    });
