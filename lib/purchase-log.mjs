/* Purchase log — shared by api/stripe-webhook.mjs (writer) and api/update-download.mjs (reader).
   ...
   Env:
     UPDATE_LOOKUP_PEPPER  — random secret, ≥32 chars, set in the Vercel project env only
                             (nothing local reads it). OPTIONAL: without it logPathFor()
                             returns null, this index is skipped entirely, and
                             api/update-download.mjs falls back to querying Stripe — which
                             still works, just with more API calls per lookup.
                             ⚠ Never change it once purchases have been indexed: the key is
                             derived from it, so a new value orphans every existing record.
