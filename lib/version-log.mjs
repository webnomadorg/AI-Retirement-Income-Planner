/* Version-check analytics — storage helpers.
   ...
   are still in use, and how fast a new release actually reaches them. Once a day the planner
   sends a fire-and-forget beacon to /api/version-ping carrying its build number; this stores
   it.

   ⚠ The beacon is deliberately SEPARATE from the update check (/api/latest-version), which
   is left byte-for-byte as it always was. Writing from inside that endpoint would put a
   storage dependency inside the customer's "Update available" button — see the long note at
   the top of Website/api/version-ping.mjs. Do not move this back.
   ...
   ⚠ ABUSE: /api/version-ping is public and unauthenticated, so a determined caller can
   inflate the store by hammering it. The build number is range-checked, which bounds how many
   directories can be created, but not the volume. The real mitigation is a Vercel WAF
   rate-limit rule on that path — see Plans/Vercel-Hardening.md. Because the beacon is its own
   endpoint, that rule cannot reach the update check however it is configured, and the instant
   kill switch is deleting api/version-ping.mjs.
