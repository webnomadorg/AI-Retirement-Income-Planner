...
export async function purgeExpired() {
  ...
  for (const prefix of [PREFIX, CLAIM_PREFIX]) {
    try {
      const blobs = await listAll({ prefix, token: token() });
      const stale = (blobs || [])
...
export async function overview(days = 30) {
  ...
  const blobs = await listAll({ prefix: PREFIX, token: token() });
...
/* The cap on how many matching records are READ. Matching itself is free (the reason and
   event are in the pathname); turning a match into a row the owner can act on is not, at
   roughly 350ms each.

   ⚠ THE COUNT AND THE CAP ARE REPORTED SEPARATELY, and that is the whole point. Filtering
   `honeypot` over 30 days can match a thousand records — reading those would take six
   minutes behind a spinner, and a tab that hangs gets treated as broken. So the answer is
   always "N matched, here are the newest 50", never a silent truncation. */
const FILTER_READ_CAP = 50;
const FILTER_READ_MAX = 200;

/* Records matching a reason and/or a set of events, newest first.

   WHY THIS EXISTS: the reason table on the Signups tab counts blocks but cannot name them,
   and a count is not actionable. `prior-honeypot: 3` tells you nothing about whether those
   three were bots or three real people the guard just cost you. The addresses do.

   ⚠ PATHNAME FIRST, READS SECOND — this ordering is what makes it affordable. Every filter
   term (day, event, reason) is already in the blob name, so narrowing costs ONE listAll()
   and zero reads, however much history exists. Only the survivors are fetched. Invert it
   (read everything, then filter in JS) and this becomes the slowest call in the console.

   ⚠ Unreadable records are COUNTED, not skipped silently. A filter that quietly returned 48
   of 50 would be lying in exactly the direction that matters here — the missing two could be
   the false positive being looked for.

   @param {object} opts
   @param {string}   [opts.reason]  exact block reason, e.g. 'prior-honeypot'
   @param {string[]} [opts.events]  e.g. ['blocked','pending'] — anything never sent to MailerLite
   @param {number}   [opts.days]    window, defaults to full retention
   @param {number}   [opts.limit]   how many to READ (not to match) */
export async function filterRecords({ reason = '', events = [], days = RETENTION_DAYS, limit = FILTER_READ_CAP } = {}) {
  const cap = Math.min(Math.max(parseInt(limit, 10) || FILTER_READ_CAP, 1), FILTER_READ_MAX);
  const wantEvents = new Set((events || []).filter(Boolean));
  const empty = {
    configured: false, reason, events: [...wantEvents], days, limit: cap,
    matched: 0, returned: 0, capped: false, unreadable: 0, records: [],
  };
  ...
  let blobs = [];
  try {
    // ⚠ listAll(), never a bare list() — it stops at 1000 and drops the NEWEST records.
    blobs = await listAll({ prefix: PREFIX, token: token() });
  } catch (err) {
    console.error('[signup-log] filterRecords list failed:', err?.message || err);
  ...
  for (const b of blobs || []) {
    ...
    if (rec.day < cutoff) continue;
    if (reason && rec.reason !== reason) continue;
    if (wantEvents.size && !wantEvents.has(rec.event)) continue;
    matches.push(rec);
  }
  // Newest first: a filter is nearly always asking "what just happened?".
  matches.sort((a, b) => (b.epoch || 0) - (a.epoch || 0));

  const head = matches.slice(0, cap);
  let unreadable = 0;
  const read = await mapLimitLocal(head, 8, async (m) => {
    try {
      ...
      if (!r || r.statusCode !== 200 || !r.stream) { unreadable++; return null; }
      const rec = JSON.parse(await new Response(r.stream).text());
      return { ...rec, day: m.day, event: m.event, reason: m.reason };
    } catch { unreadable++; return null; }
  });

  const records = read.filter(Boolean);
  return {
    configured: true,
    reason,
    events: [...wantEvents],
...
/* Local copy of the bounded-concurrency helper. Deliberately NOT imported from the console's
   nurture.mjs: this module ships inside the WEBSITE bundle and must not reach into the admin
   tool. ⚠ Unlike that one, a rejection here is caught per item and counted — see above. */
...
/* Full records for ONE day. The only call that fetches contents, and it is made when the
   ...
   much history exists. */
/*
 * Blob reads run in a bounded pool rather than one after another.
 *
 * ⚠ THIS IS THE WHOLE COST of every read path in this file. There is ONE BLOB PER RECORD and a
 * private read is ~170ms, so reading them in sequence scales with the number of RECORDS, not the
 * number of days: 524 records took 87 seconds and the console's search box looked hung. Pooled,
 * the same read is a few seconds.
 *
 * ⚠ Ordering is unaffected -- results are written back into their own slots and every caller
 * sorts what it gets -- and each read keeps its OWN try/catch, so one unreadable blob still
 * cannot take a whole day down with it. Both properties were true before; neither is free.
 *
 * ⚠ THE WIDTH IS A RATE-LIMIT DECISION, not a performance dial. Vercel Blob allows 120 simple
 * operations per SECOND on Pro (7,200/min); a blob read is one. Measured, a pool of 12 sustained
 * ~45 reads/sec -- about 38% of the ceiling from a single search, and this console can easily
 * have a day loading in another panel while a search runs. 8 sustains ~30/sec, a quarter of the
 * limit, which leaves room for the console to be doing something else at the same time and costs
 * only a few seconds. Do not raise it to make a search feel faster: the answer to that is the
 * day cache below, which removes the reads rather than compressing them.
 */
const READ_POOL = 8;

/*
 * Completed days are cached for the life of the process.
 *
 * This is the change that actually REDUCES blob operations rather than rearranging them. A day
 * in the past cannot gain or lose records -- each record is its own blob under that day's prefix
 * and nothing rewrites history -- so re-reading it on the next search buys nothing and spends
 * another few hundred operations. A second search across the same window now costs almost none.
 *
 * ⚠ TODAY IS NEVER CACHED. It is the one day that is still being written to, and serving a
 * stale copy of it would mean a signup that just happened not showing up -- the single answer
 * this panel must never get wrong.
 *
 * ⚠ Deliberately no expiry and no size cap: the console is a short-lived local process, and the
 * whole retention window is a few hundred small records. In a long-running server this would
 * need bounding.
 */
...
/*
 * An OPTIONAL second tier that survives the process, supplied from outside.
 *
 * ⚠ The implementation deliberately does NOT live here. This module is imported by the live
 * signup, confirm and contact endpoints, and they must not acquire a filesystem dependency for
 * something only the local admin console uses -- nor could they write to a disk that persists.
 * So this is a nullable hook: production never calls useDayCache(), `disk` stays null, and the
 * behaviour is exactly what it was. The console installs one at startup and owns the file
 * format, the location and -- the part that matters -- the pruning.
 *
 * ⚠ WHATEVER IS INSTALLED HERE HOLDS REAL ADDRESSES AND IP ADDRESSES. RETENTION_DAYS is the
 * privacy control for this data; a cache that outlives it makes the retention promise false.
 * Any implementation must prune on the same 90-day rule.
 */
let disk = null;

/** Install a persistent day cache: `{ read(day), write(day, records) }`. Admin-side only. */
export function useDayCache(impl) { disk = impl || null; }
...
/** One record, or null when it cannot be read. Shared by readDay and the search fast path. */
...
export async function readDay(day) {
  ...
  const memo = dayCache.get(day);
  if (memo) return memo;
  /* Disk before the store, but only for a day that is finished -- see the today() guard below
     for why. A failed or absent read just falls through and costs nothing but the attempt. */
  if (disk && day < today()) {
    const saved = await disk.read(day);
    if (saved) { dayCache.set(day, saved); return saved; }
  }
  const blobs = await listAll({ prefix: `${PREFIX}${day}/`, token: token() });
  const recs = await mapPool(blobs || [], READ_POOL, (b) => readRecord(b.pathname));
  const out = recs
    .filter(Boolean)
    // v1 records predate the full-address change and only carry a masked form.
    .map((rec) => ({ ...rec, email: rec.email || rec.masked || '(not recorded)' }))
    .sort((a, b) => String(b.at).localeCompare(String(a.at)));
  if (day < today()) {
    dayCache.set(day, out);
    if (disk) await disk.write(day, out);
  }
  return out;
}
...
/* 'prior-honeypot' is in here so the panel stays coherent with the guard: once we have
   REFUSED someone for automation, a verdict of 'clean' on the same person would be a
   flat contradiction. It matters at the window edges, where the original honeypot rows
   have aged out of the 30-day block window but the refusals they caused have not. */
const AUTOMATED_REASONS = new Set(['honeypot', 'submitted-too-fast', 'token-forged', 'prior-honeypot']);

/** Several submissions this close together are not a person changing their mind. */
const BURST_MS = 60_000;
const BURST_COUNT = 3;

/* ------------------------------------------------- prior-honeypot block --- */

/* How far back a honeypot hit disqualifies an address, and how many hits it takes.

   WHY THIS RULE EXISTS AT ALL — the per-request honeypot does NOT stop these bots.
   The script fills `_honey` on most attempts and leaves it alone on exactly one, and that
   one submission is clean by construction, so a per-request check can never see it. What it
   cannot hide is the history: all eight bots found in the 21 Aug backfill tripped the
   honeypot five to twelve times on the SAME address they then confirmed, inside two minutes.
   That is what this reads.

   ⚠ THE FALSE-POSITIVE CASE IS NARROWER THAN IT LOOKS, and the reasoning matters because
   this is a real block on a real form. The stock objection is that an aggressive password
   manager can fill a hidden field — true, and it is why the Nurture flag never acts. But a
   password manager that fills `_honey` fills it on EVERY submission, so that visitor is
   already permanently blocked by the existing per-request check; this rule adds them no new
   harm. The only person it newly costs is someone who tripped the honeypot once and then
   submitted cleanly, which is why the threshold is TWO and not one. `_honey` is also not a
   name autofill reaches for, unlike `email` or `name`.

   ⚠ The window is deliberately far shorter than the 90-day retention. A wrongly-blocked
   person recovers on their own after this many days; at 90 they would effectively be gone
   for good, and they never complain to tell us. Catching the bots does not need the length —
   every observed run finished inside 114 seconds. */
const PRIOR_HONEYPOT_DAYS = 30;
const PRIOR_HONEYPOT_MIN_HITS = 2;

/* The judgement, pure and separate from the storage call so it can be tested without a blob
   store — same split as verdictFor/botSignals below. Takes parsed pathnames. */
export function countHoneypotHits(records, cutoffDay) {
  let hits = 0;
  let lastAt = null;
  for (const r of records || []) {
    if (r.reason !== 'honeypot') continue;
    if (cutoffDay && r.day < cutoffDay) continue;
    hits++;
    if (r.epoch && (!lastAt || r.epoch > lastAt)) lastAt = r.epoch;
  }
  return { hits, lastAt };
}

/* Has this address tripped the honeypot often enough, recently enough, to refuse it?

   ⚠ ONE list() CALL, ZERO RECORD READS. The reason lives in the pathname, so this costs the
   same whether the store holds ten records or ten thousand — the constraint that the whole
   log layout exists to satisfy. Do not be tempted to read the records for "more signal".

   ⚠ FAILS OPEN, ALWAYS. If the blob store is unconfigured or list() throws, this returns
   false and the signup proceeds. A spam rule that converts an infrastructure blip into a
   dead signup form is a worse outcome than the spam it prevents, and unlike the spam it is
   invisible. Contrast SIGNUP_TOKEN_SECRET, which fails CLOSED on purpose — that one guards
   the token's integrity, this one is a heuristic on top.

   ⚠ Pass the CANONICAL address. keyId() collapses dotted Gmail variants, so the caller must
   have run canonicaliseEmail() first or a bot's forty faces stay forty separate histories. */
export async function priorHoneypot(canonicalEmail, days = PRIOR_HONEYPOT_DAYS) {
  const none = { blocked: false, hits: 0, lastAt: null };
  const kid = keyId(canonicalEmail);
  if (!isConfigured() || !kid) return none;
  ...
  try {
    ...
    console.error('[signup-log] priorHoneypot list failed:', err?.message || err);
    return none;                                  // fail open — see above
  }

  const mine = (blobs || [])
    .map((b) => parseKey(b.pathname))
    .filter((r) => r.kid === kid && r.event === 'blocked');

  const { hits, lastAt } = countHoneypotHits(mine, cutoff);
  return { blocked: hits >= PRIOR_HONEYPOT_MIN_HITS, hits, lastAt };
}
...
export async function botSignals(emails, days = RETENTION_DAYS) {
  ...
  const cutoff = new Date(Date.now() - days * 86400_000).toISOString().slice(0, 10);
  let blobs = [];
  try { blobs = await listAll({ prefix: PREFIX, token: token() }); }
...
export async function search(term, days = RETENTION_DAYS) {
  ...
  /* FAST PATH: a whole email address needs no scan at all.

     The key id is derived from the address, and it is in every pathname — so "show me
     everything for this person" is a list() plus a handful of reads, not a walk of the window.
     At ~350ms per read, the slow path below is about two minutes today and gets worse every
     week; this is a couple of seconds and stays that way.

     ⚠ Only whole, parseable addresses qualify. keyId() returns '' for a domain or a fragment,
     which is exactly when the scan is still needed — so this never silently answers a broader
     question with a narrower search. It is also canonical, so pasting any dotted Gmail variant
     finds all of them, same as the slow path. */
  const kid = keyId(q);
  if (kid) {
    const cutoff = new Date(Date.now() - days * 86400_000).toISOString().slice(0, 10);
    let blobs = [];
    ...
    const mine = blobs.map((b) => parseKey(b.pathname)).filter((r) => r.kid === kid && r.day >= cutoff);
    /* No records for this key id? Only fall through to the scan if the window still holds
       records that HAVE no key id — those are the only ones the fast path could have missed.

       ⚠ Checked, never assumed. After the 21 Aug 2026 backfill every record carries a key id,
       so this is normally provable in memory from the listing already fetched, and a search
       that finds nothing returns in two seconds instead of scanning for a minute to reach the
       same answer. If an unkeyed record ever appears again, the scan quietly comes back. */
    if (!mine.length && !blobs.some((b) => !parseKey(b.pathname).kid)) {
      return { term: q, tooShort: false, byKeyId: true, daysSearched: null, hits: [] };
    }
    if (mine.length) {
      const read = await mapPool(mine, READ_POOL, async (m) => {
        const rec = await readRecord(m.pathname);
        return rec ? { ...rec, day: m.day } : null;
      });
