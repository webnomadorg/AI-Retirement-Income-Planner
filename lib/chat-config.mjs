...
/* ⚠ AFTER WE WRITE, OUR OWN COPY IS THE TRUTH -- DO NOT RE-READ TO CONFIRM IT.
   A blob takes a noticeable time to become readable: measured here at nearly a minute, far
   longer than the ~2.4s this project's notes quote. So a read straight after a write hands
   back the PREVIOUS document. The console does exactly that -- Save, then Load -- and it
   made a successful save look like it had been ignored, with the old values reappearing in
   the form.

   `fresh` therefore means "do not serve a stale READ", not "go and read again regardless".
   When the cached document is one we wrote ourselves, it is newer than anything a read
   could return, and it wins. AUTHORITATIVE_MS is generous for that reason. */
const AUTHORITATIVE_MS = 5 * 60_000;
