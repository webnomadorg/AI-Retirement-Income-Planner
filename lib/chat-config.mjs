...
export const DEFAULTS = {
  ...
  houseNotes: '',
  /* Test mode. With this on the assistant is fully live — it answers, it spends — but it
     only appears on a URL carrying ?chatbot=on, so ordinary visitors never see it. That is
     what makes it possible to try the real thing on the real site before anyone else can.
     ⚠ It is a curtain, not a lock: anyone who knows the parameter can use it. The caps and
     the spend ceiling still apply, and they are what actually bound the cost. */
  previewOnly: false,
...
let _cache = null;
let _cacheAt = 0;
let _cacheIsOurWrite = false;

/* ⚠ AFTER WE WRITE, OUR OWN COPY IS THE TRUTH -- DO NOT RE-READ TO CONFIRM IT.

   A blob takes a moment to become readable. Measured on this store 2026-09-08 over four
   overwrites: 0.51s, 0.52s, 0.51s, and 1.35s on the first write of the session. The console
   saves and immediately reloads, which lands inside that window, and the reload then hands
   back the PREVIOUS document -- making a successful save look as though it had been ignored.

   `fresh` therefore means "do not serve a stale READ", not "go and read again regardless".
   A document we wrote ourselves is newer than anything a read can return, so it wins.

   ⚠ An earlier version of this comment claimed the lag was nearly a minute. That was a bad
   inference from a messy test with two server processes running, not a measurement. The real
   figure is under two seconds; the window below is sized for that with room to spare. */
const AUTHORITATIVE_MS = 30_000;

export async function readConfig({ fresh = false } = {}) {
  const now = Date.now();
  if (_cache && _cacheIsOurWrite && now - _cacheAt < AUTHORITATIVE_MS) return _cache;
  if (!fresh && _cache && now - _cacheAt < TTL_MS) return _cache;
  _cache = await readRaw();
  _cacheAt = now;
  _cacheIsOurWrite = false;
  return _cache;
}
...
export async function writeConfig(next) {
  ...
  /* Update from what we just wrote rather than re-reading -- see readConfig above. */
  _cache = doc;
  _cacheAt = Date.now();
  _cacheIsOurWrite = true;
  return doc;
}
 ...
 *  `preview` is true when the visitor's URL carried ?chatbot=on. */
export function showsOn(cfg, pathname, preview = false) {
  if (!cfg || !cfg.enabled) return false;
  // In test mode the placement list is ignored entirely: the parameter IS the placement.
  if (cfg.previewOnly) return preview === true;
