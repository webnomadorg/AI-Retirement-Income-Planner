...
**Meta description:** A retirement plan has two jobs: produce income now and leave enough for the next phase. Here is why the ending balance is the number that links your phases together, how to read it by account type, and how the planner shows it.

**Suggested URL slug:** retirement-ending-balance-that-carries-the-plan

**Image source base:** retirement-ending-balance-that-carries-the-plan
...
**Secondary keywords:** portfolio ending balance, retirement drawdown, sequence of returns risk, planned drawdown vs depletion, retirement account mix
...
[IMAGE retirement as a chain of balances passed phase to phase 1]
...
This is the fourth article in our framework series drawn from the free companion eBook. The previous one covered [gross, net, and real income, and reliable versus flexible income](/blog/gross-net-real-retirement-income.html).
...
If those withdrawals draw the 401(k) down too fast, the retiree has fewer assets once Medicare begins. If they also lift taxable income, they can raise [ACA costs before 65](/blog/how-much-can-i-spend-in-retirement.html). And selling more early can reduce the base that would have compounded later. The income target was reached, but the plan may have paid too much to reach it.
...
[IMAGE planner Balance tab account balances chart with insights popover 2]
...
**Soft CTA:** Want to see the ending balance carried from one phase to the next? The interactive demo shows a full sample plan where each phase card reports its ending balance and the Balance tab charts the whole trajectory.

**Demo CTA:** Open the live demo, look at a phase card's ending balance on the Overview tab, then open the Balance tab to see the same money charted across every phase and account.

**Product CTA:** The AI Retirement Income Planner shows the ending balance on every phase card, charts account balances and three return scenarios on the Balance tab, reads the balance by tax type, stress-tests it, and lets optional AI explain it. One-time purchase, no subscription, runs privately in your browser.

## Facebook Post Snippets

**Snippet 1:** A retirement plan has two jobs: produce income now, and leave enough for the next phase. Most plans nail the first and ignore the second, which is how a plan looks great today and fragile later. The number that links them is the ending balance.

**Snippet 2:** A phase can hit its $5,000-a-month income target and still be a problem, if it drained the 401(k) too fast to get there. In retirement, "did I hit my income number" is only half the question. The other half is "what was left afterward?"

**Snippet 3:** Two retirees both end a phase with $500,000. One is almost all pre-tax (big future RMDs and taxes); the other is split with Roth and cash (far more flexible). The ending balance is not just how much remains. It is where it remains.

## Newsletter Summary

A retirement plan has two jobs: create income for the phase you are in, and leave enough for the phase that follows. The number that connects them is the ending balance, and this article makes the case for reading it phase by phase. Retirement is a chain of balances, where each phase's ending balance is the next phase's starting balance, so an early decision ripples through everything after it. A phase can meet its income target and still weaken the future by drawing down too fast, and the difference between a sound plan and a fragile one is usually planned drawdown versus accidental depletion, which comes down to visibility. It covers sequence-of-returns risk, why the account mix of the balance (pre-tax, Roth, cash, brokerage) matters as much as the total, and how taxes and healthcare reshape future balances. Finally it shows how the planner surfaces the ending balance on every phase card, charts it by account and across three return scenarios on the Balance tab, stress-tests it, and recognizes a deliberate drawdown when guaranteed income takes over.
