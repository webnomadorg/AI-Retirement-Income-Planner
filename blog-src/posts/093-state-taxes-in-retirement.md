...
**Meta description:** Most retirement calculators model federal tax and stop. State tax can move net income by thousands a year, and states treat Social Security, pensions, and IRA withdrawals very differently. Here is what to look up, and how to put it into a plan.
...
**Primary keyword:** state taxes in retirement

**Secondary keywords:** retirement calculator by state, state tax on retirement income, do states tax Social Security, state pension exclusion, moving states in retirement

**Target reader:** Someone building a retirement income plan who has realized that a federal-only estimate is not the whole answer, and who may also be weighing a move.

**Educational disclaimer:** This article is for general education only. It is not financial, tax, investment, legal, Social Security, Medicare, estate, or retirement advice. State tax rules, exemptions, thresholds, and rates are set by each state and change regularly. Confirm current rules with your state's revenue department and a qualified professional before making decisions.
...
1. **Your effective state rate** on the income you actually expect — not the top marginal rate, which almost nobody pays on the whole amount.
2. **Whether Social Security is taxed** in your state.
3. **Whether pension and retirement-account income is exempt**, and if so, up to what cap and from what age.
4. **When those rules were last changed**, so you know how much to trust them in five years.
...
- **Survivor years.** A surviving spouse usually files single, and a state exemption that was comfortable for a couple can be far less so afterward — the same cliff that makes the federal [survivor tax picture](/blog/survivor-benefits-widows-tax-cliff.html) so sharp.

## A Quick Sanity Check

If you want the shape of the answer before doing anything careful, the free [retirement tax calculator](/retire-abroad.html) shows what tax and healthcare take out of a given plan, with no sign-up. It models the federal layer and the country-level differences rather than individual states, so read it as the floor: whatever your state adds, sits on top of that figure.
